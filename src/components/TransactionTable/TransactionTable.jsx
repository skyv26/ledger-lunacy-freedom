...
const removeDuplicateData = (arrayOfObjects) => {
    ...
    newArrayOfObject = newArrayOfObject.sort((a, b) => new Date(b.date) - new Date(a.date));
    ...
    return (
        <table>
            <thead>
                <tr>
                    {
                        DATA_HEADINGS.map((headings, index) => <th key={`${headings}-${index+1}`}>{headings}</th>)
                    }
                </tr>
            </thead>
            <tbody>
                {
                    removeDuplicateData(DATA).map((data) => <TransactionRow key={data.activity_id} data={data} /> )
                }
            </tbody>
        </table>
    );
